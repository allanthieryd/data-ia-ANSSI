...
def load_and_prepare_data():
    """Charge le CSV et prépare les données pour le ML."""
    ...
    df_ml = df[(df["CVE"] != "") & df["CVSS"].notna() & df["EPSS"].notna()].copy()
...
def clustering_analysis(df_ml):
    ...
    plt.tight_layout()
    plt.savefig("ml_elbow_silhouette.png", dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("ml_clustering_pca.png", dpi=150)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig("ml_clustering_cvss_epss.png", dpi=150)
    plt.show()
...
def supervised_classification(df_ml):
    ...
    if len(df_sup) < 10:
        ...
        print("   Essayez d'augmenter MAX_BULLETINS dans Data_IA.py")
        return
    ...
    plt.tight_layout()
    plt.savefig("ml_confusion_matrix.png", dpi=150)
    plt.show()
    ...
    if len(target_names) >= 2:
        try:
            ...
            ax.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig("ml_roc_curves.png", dpi=150)
            plt.show()
    ...
    plt.tight_layout()
    plt.savefig("ml_distribution_pred_vs_real.png", dpi=150)
    plt.show()
...
def plot_cluster_severity_distribution(df_ml):
    ...
    plt.tight_layout()
    plt.savefig("ml_cluster_severity.png", dpi=150)
    plt.show()


def plot_cluster_cwe_analysis(df_ml):
    """Top CWE par cluster."""
    data = df_ml[(df_ml["CWE"] != "Non disponible") & df_ml["CWE"].notna()]
    if data.empty or "Cluster" not in data.columns:
        return
    ...
    plt.tight_layout()
    plt.savefig("ml_cluster_cwe.png", dpi=150)
    plt.show()
...
if __name__ == "__main__":
    ...
    if len(df_ml) < 5:
        ...
        print("   Augmentez MAX_BULLETINS dans Data_IA.py et relancez l'étape 4.")
    else:
        ...
        print("Fichiers sauvegardés : ml_*.png")
        print("=" * 60)
